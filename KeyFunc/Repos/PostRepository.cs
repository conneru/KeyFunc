...
namespace KeyFunc.Repos
{
    public class PostRepository : Repository<Post>, IPostRepository
    {
        ...
        public PostRepository(KeyFuncContext context)
            : base(context)
        {
            _context = context;
        }
        ...
        public async Task<Post?> GetPostDetails(int Id)
        {
            Post? post = await _context
                ...
                .Include(p => p.User)
                .Include(p => p.Images.OrderBy(i => i.OrderNum))
                .SingleAsync();
        ...
        public async Task<IEnumerable<Post>?> GetFollowingPosts(User user)
        {
            ...
            List<Post>? followerPosts = await _context
                .Posts.Where(p => p.User.Followers.Contains(u))
                .Include(p => p.User)
                .Include(p => p.Comments)
                .Include(p => p.Images)
